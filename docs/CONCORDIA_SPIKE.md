...
Паттерн реализован как отдельный режим «Практическая миссия»: 10 сценариев B2,
не более трёх ходов, один вызов Qwen2.5-3B на ход и отдельная таблица
`mission_attempts`. Финальный benchmark: 20/20 детерминированных проверок и 11/11
живых сценариев; средняя задержка хода 11,7 секунды. Runtime Concordia не является
