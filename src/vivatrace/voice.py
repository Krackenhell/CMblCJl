...
<div class="shell">
  ...
  <div class="controls"><button id="start" class="primary">Начать диалог</button><button id="manual" class="secondary" disabled>Начать реплику вручную</button><button id="stop" class="danger" disabled>Завершить диалог</button><div class="hint">Реплика отправится после тишины. Если автоопределение не сработало, нажмите «Отправить реплику».</div></div>
</div>
<script>
const CONFIG=__CONFIG__;
const SAMPLE_RATE=16000,SILENCE_MS=850,MAX_UTTERANCE_MS=15000;
let ws,audioContext,stream,source,processor,zeroGain,speech=false,awaitingResponse=false,speechStartedAt=0,lastVoiceAt=0,hotFrames=0,quietFrames=0,noiseFloor=0.008,preRoll=[],currentAudio=null,running=false,botSpeaking=false;
...
function beginSpeech(){if(speech||awaitingResponse||!running)return;speech=true;speechStartedAt=Date.now();lastVoiceAt=Date.now();quietFrames=0;if(botSpeaking){stopPlayback();sendJson({type:'interrupt'})}sendJson({type:'speech_start'});preRoll.forEach(chunk=>ws.send(chunk.buffer));preRoll=[];manualBtn.textContent='Отправить реплику';manualBtn.className='primary';setStatus('Записываю реплику… Сделайте паузу или нажмите «Отправить реплику».',true)}
function endSpeech(reason='manual'){if(!speech)return;speech=false;awaitingResponse=true;hotFrames=0;quietFrames=0;sendJson({type:'speech_end'});manualBtn.disabled=true;manualBtn.textContent='Реплика отправлена';manualBtn.className='secondary';setStatus(reason==='timeout'?'Достигнут лимит 15 секунд. Распознаю реплику…':'Распознаю и формирую ответ…',true)}
