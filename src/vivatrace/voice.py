...
.meter{height:7px;flex:1;background:#e7ede9;border-radius:10px;overflow:hidden}.meter i{display:block;height:100%;width:0;background:linear-gradient(90deg,#087256,#baff22,#60bdf5);transition:width .08s}
.chat{height:405px;overflow:auto;padding:18px 20px;background:radial-gradient(circle at 20% 20%,rgba(186,255,34,.06),transparent 15rem),#f8fbf9}.msg{max-width:84%;padding:12px 15px;border-radius:18px;margin:0 0 12px;line-height:1.42;font-size:13px;white-space:pre-wrap;box-shadow:0 4px 13px rgba(5,61,49,.04)}.bot{background:white;border:1px solid #e0e9e3;border-bottom-left-radius:6px}.student{background:linear-gradient(135deg,#075844,#0c7156);color:white;margin-left:auto;border-bottom-right-radius:6px}.system{background:#fff4dc;color:#654d13;margin-left:auto;margin-right:auto;font-size:11px}.feedback{margin:-6px 0 14px auto;max-width:84%;font-size:11px;color:#43584f;border-left:3px solid #baff22;padding:8px 10px;background:#f2f9e8;border-radius:0 10px 10px 0}
.voice-pad{margin-top:auto;padding:13px 18px 16px;border-top:1px solid #e4ebe6;background:#fff}.listen{display:flex;justify-content:space-between;align-items:center;margin-bottom:9px}.listen b{font-size:12px;color:#075844}.listen span{font-size:10px;color:#74827c}.controls{display:flex;gap:8px;align-items:center}.controls button{border:0;border-radius:13px;padding:11px 14px;font-weight:800;cursor:pointer;font-size:11px}.primary{background:linear-gradient(90deg,#aefa25,#c8ff37);color:#103d30;box-shadow:0 7px 18px rgba(139,204,45,.2)}.secondary{background:#edf4ef;color:#174d3b}.danger{background:#fff0ee;color:#b74135}.controls button:disabled{opacity:.42;cursor:not-allowed}.hint{font-size:10px;color:#75837d;line-height:1.35;margin-top:8px}
.aside-kicker{font-size:9px;letter-spacing:.11em;font-weight:900;color:#11845d}.analysis h3{font-size:17px;margin:5px 0 14px;letter-spacing:-.02em}.scores{display:grid;grid-template-columns:1fr 1fr;gap:8px}.score{background:#f1f6f2;border:1px solid #e1e9e3;border-radius:13px;padding:10px}.score b{display:block;font-size:17px;color:#10251e}.score span{font-size:9px;color:#68776f}.insight-card,.goal-card{border:1px solid #dfe8e2;border-radius:15px;padding:12px;margin-top:11px;background:#fff}.insight-card b,.goal-card b{font-size:11px}.insight-card p,.goal-card p{font-size:10px;line-height:1.45;color:#5e7067;margin:6px 0 0}.goal-line{height:7px;border-radius:99px;background:#e7ede9;overflow:hidden;margin:9px 0 4px}.goal-line i{display:block;width:0;height:100%;background:linear-gradient(90deg,#075844,#baff22);transition:width .3s}.privacy{font-size:9px;color:#6d7c75;margin-top:12px;display:flex;gap:6px}.privacy:before{content:'✓';color:#11845d;font-weight:900}
@media(max-width:760px){.shell{height:720px}.workspace{grid-template-columns:1fr}.analysis{display:none}.top{height:88px}.workspace{height:632px}.chat{height:410px}.local{display:none}}
</style>
</head>
<body>
<div class="shell">
  ...
  <div class="workspace">
    <section class="dialogue">
      ...
      <div class="voice-pad"><div class="listen"><b>Голосовая реплика</b><span>пауза до 5 секунд остаётся внутри мысли</span></div><div class="controls"><button id="start" class="primary">● Начать разговор</button><button id="manual" class="secondary" disabled>Записать вручную</button><button id="stop" class="danger" disabled>Завершить</button></div><div class="hint">Говорите естественно. Если автоматическая отправка мешает, используйте ручную запись.</div></div>
    </section>
    <aside class="analysis"><div class="aside-kicker">ОБРАТНАЯ СВЯЗЬ</div><h3>Разговорный прогресс</h3><div id="scores" class="scores"><div class="score"><b>—</b><span>Грамматика</span></div><div class="score"><b>—</b><span>Словарь</span></div><div class="score"><b>—</b><span>По теме</span></div><div class="score"><b>—</b><span>Беглость</span></div></div><div class="insight-card"><b>Последний сигнал</b><p id="insight">После первой содержательной реплики здесь появится точечная обратная связь.</p></div><div class="goal-card"><b>Цель сессии</b><p>Дать собственный пример и объяснить выбор формы.</p><div class="goal-line"><i id="goalProgress"></i></div><p id="goalCopy">0 из 2 шагов подтверждено</p></div><div class="privacy">Аудио и ответы обрабатываются на этом устройстве</div></aside>
  </div>
</div>
<script>
...
function addFeedback(data){if(!data||data.scoring_available===false)return;const node=document.createElement('div');node.className='feedback';const audit=(data.grammar_findings||[]).length?' Проверено: независимое правило + локальная Qwen.':(data.structural_evidence||[]).length?' Проверено: структурный анализ relative clause + локальная Qwen.':' Проверено: локальная Qwen.';node.textContent=data.feedback_ru+(data.correction_en?' Исправление: '+data.correction_en:'')+audit;chat.appendChild(node);chat.scrollTop=chat.scrollHeight;const metrics=data.metrics||{};const scores=[['Грамматика',data.grammar_score],['Словарь',data.vocabulary_score],['По теме',data.relevance_score],['Беглость',metrics.fluency_score]];const box=document.getElementById('scores');box.innerHTML='';scores.forEach(([name,value])=>{const n=document.createElement('div');n.className='score';n.innerHTML='<b>'+Math.round((value||0)*100)+'%</b><span>'+name+'</span>';box.appendChild(n)});const insight=document.getElementById('insight');if(insight)insight.textContent=data.feedback_ru+(data.correction_en?' Исправление: '+data.correction_en:'');const target=Math.max(Number(data.grammar_score)||0,Number(data.relevance_score)||0);const goal=document.getElementById('goalProgress'),copy=document.getElementById('goalCopy');if(goal)goal.style.width=(target>=.75?'100%':'50%');if(copy)copy.textContent=target>=.75?'2 из 2 шагов подтверждено':'1 из 2 шагов подтверждено'}
...
.chat{height:405px;overflow:auto;padding:18px 20px;background:radial-gradient(circle at 20% 20%,rgba(96,189,245,.07),transparent 15rem),#f8fbf9}.msg{max-width:84%;padding:12px 15px;border-radius:18px;margin:0 0 12px;line-height:1.42;font-size:13px;white-space:pre-wrap;box-shadow:0 4px 13px rgba(5,61,49,.04)}.bot{background:white;border:1px solid #e0e9e3;border-bottom-left-radius:6px}.bot.streaming{opacity:.74}.student{background:linear-gradient(135deg,#075844,#0c7156);color:white;margin-left:auto;border-bottom-right-radius:6px}.system{background:#fff4dc;color:#654d13;margin-left:auto;margin-right:auto;font-size:11px}.feedback{margin:-6px 0 14px auto;max-width:84%;font-size:11px;color:#43584f;border-left:3px solid #baff22;padding:8px 10px;background:#f2f9e8;border-radius:0 10px 10px 0}.feedback.pending{color:#748078;border-left-color:#cdd7d0;background:#f5f7f5}
.voice-pad{margin-top:auto;padding:13px 18px 16px;border-top:1px solid #e4ebe6;background:#fff}.listen{display:flex;justify-content:space-between;align-items:center;margin-bottom:9px}.listen b{font-size:12px;color:#075844}.listen span{font-size:10px;color:#74827c}.controls{display:flex;gap:8px;align-items:center}.controls button{border:0;border-radius:13px;padding:11px 14px;font-weight:800;cursor:pointer;font-size:11px}.primary{background:linear-gradient(90deg,#aefa25,#c8ff37);color:#103d30;box-shadow:0 7px 18px rgba(139,204,45,.2)}.danger{background:#fff0ee;color:#b74135}.controls button:disabled{opacity:.42;cursor:not-allowed}.hint{font-size:10px;color:#75837d;line-height:1.35;margin-top:8px}
.aside-kicker{font-size:9px;letter-spacing:.11em;font-weight:900;color:#11845d}.analysis h3{font-size:17px;margin:5px 0 14px;letter-spacing:-.02em}.scores{display:grid;grid-template-columns:1fr 1fr;gap:8px}.score{background:#f1f6f2;border:1px solid #e1e9e3;border-radius:13px;padding:10px}.score b{display:block;font-size:17px;color:#10251e}.score span{font-size:9px;color:#68776f}.insight-card,.goal-card{border:1px solid #dfe8e2;border-radius:15px;padding:12px;margin-top:11px;background:#fff}.insight-card b,.goal-card b{font-size:11px}.insight-card p,.goal-card p{font-size:10px;line-height:1.45;color:#5e7067;margin:6px 0 0}.goal-line{height:7px;border-radius:99px;background:#e7ede9;overflow:hidden;margin:9px 0 4px}.goal-line i{display:block;width:0;height:100%;background:linear-gradient(90deg,#075844,#60bdf5,#baff22);transition:width .3s}.privacy{font-size:9px;color:#6d7c75;margin-top:12px;display:flex;gap:6px}.privacy:before{content:'✓';color:#11845d;font-weight:900}
@media(max-width:760px){.shell{height:720px}.workspace{grid-template-columns:1fr}.analysis{display:none}.top{height:88px}.workspace{height:632px}.chat{height:410px}.cloud{display:none}}
</style>
</head>
<body>
<div class="shell">
...
function addAssessment(result,turn){const assessment=result.assessment||{},metrics=result.metrics||{},node=turn.feedbackNode;if(result.skipped||!assessment.scoring_available){if(node)node.remove();return}const quote=assessment.evidence_quote?' Основание: «'+assessment.evidence_quote+'».':'';node.className='feedback';node.textContent=assessment.feedback_ru+(assessment.correction_en?' Исправление: '+assessment.correction_en:'')+quote;chat.scrollTop=chat.scrollHeight;const scores=[['Грамматика',assessment.grammar_score],['Словарь',assessment.vocabulary_score],['По теме',assessment.relevance_score],['Беглость',metrics.fluency_score]];const box=document.getElementById('scores');box.innerHTML='';scores.forEach(([name,value])=>{const n=document.createElement('div');n.className='score';n.innerHTML='<b>'+Math.round((value||0)*100)+'%</b><span>'+name+'</span>';box.appendChild(n)});const insight=document.getElementById('insight');if(insight)insight.textContent=assessment.feedback_ru+(assessment.correction_en?' Исправление: '+assessment.correction_en:'');const target=Math.max(Number(assessment.grammar_score)||0,Number(assessment.relevance_score)||0);const goal=document.getElementById('goalProgress'),copy=document.getElementById('goalCopy');if(goal)goal.style.width=(target>=.75?'100%':'50%');if(copy)copy.textContent=target>=.75?'2 из 2 шагов подтверждено':'1 из 2 шагов подтверждено'}
